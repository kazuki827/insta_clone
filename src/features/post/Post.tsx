...
const Post: React.FC<PROPS_POST> = ({ postId, loginId, userPost, title, imageUrl, liked }) => {
  ...
  // storeから得られるcommentsからpostIdに対応したcommentだけ抜き出す。
  const commentsOnPost = comments.filter((com) => {
    return com.post === postId;
  });
  
  // storeから得られるprofilesから投稿したuserのprofileだけ抜き出す。
  ...
  if (title) {
    return (
      <div className={styles.post}>
        ...
        <img className={styles.post_image} src={imageUrl} alt="投稿画像" />
        ...
        <div className={styles.post_comments}>
          {commentsOnPost.map((comment) => (
            <div key={comment.id} className={styles.post_comment}>
              <Avatar
                src={profiles.find((prof) => prof.userProfile === comment.userComment)?.img}
        ...
        <form className={styles.post_commentBox}>
          ...
          />
          <button disabled={!text.length} className={styles.post_button} type="submit" onClick={postComment}>
            Post
          </button>
        </form>
      </div>
